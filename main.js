...
(function(){
  ...
  var atkLogMsgs = [
    '⚔  Khukuri barrage — ALL DEFLECTED!',
    '💀  Multi-angle assault — BLOCKED!',
    '⚡  Coordinated strike — COUNTERED!',
    '🩸  Surround attack — NEUTRALIZED!',
    '☠️  Final barrage — REPELLED!',
    '🔥  Simultaneous throw — SHIELDED!',
    '🛡️  Saroj holds the line!',
    '🇳🇵  One Gurkha. Many enemies. No fear.',
  ];
  var atkLogIdx = 0;

  function spawnKnifeFrom(atk) {
    if(battleDone || atk.defeated) return;
    var defX = bc.width  * defender.ex;
    var defY = bc.height * defender.ey;
    var ax   = bc.width  * atk.ex;
    var ay   = bc.height * atk.ey;
    knives.push({
      x: ax, y: ay,
      tx: defX + (Math.random()-0.5)*50,
      ty: defY + (Math.random()-0.5)*60,
      speed: 4 + Math.random()*3,
      angle: Math.atan2(defY-ay, defX-ax),
      spin: (Math.random()>0.5?1:-1) * (0.18 + Math.random()*0.22),
      hit: false, alpha: 1,
      color: atk.color, atkId: atk.id,
    });
  ...
  function drawShield(x, y, t) {
    ...
    ctx.shadowBlur = 18 + Math.sin(t*2)*8;
    ctx.strokeStyle='#00ff41'; ctx.lineWidth=2.5;
    ctx.fillStyle='rgba(0,255,65,.1)';
    ctx.beginPath();
    ctx.moveTo(0,-36); ctx.lineTo(26,-20); ctx.lineTo(26,16);
    ctx.lineTo(0,36);  ctx.lineTo(-26,16); ctx.lineTo(-26,-20);
    ctx.closePath();
    ctx.fill(); ctx.stroke();
    ctx.shadowBlur=0;
    ctx.restore();
  ...
  function drawDefeatedX(x, y) {
    ctx.save();
    ctx.strokeStyle='rgba(255,51,51,.5)'; ctx.lineWidth=3;
  ...
  function loop() {
    ...
    var defX = W * defender.ex;
    var defY = H * defender.ey;

    // ground line
    var gy = H * 0.72;
    var grad = ctx.createLinearGradient(0,gy,0,gy+18);
    grad.addColorStop(0,'rgba(0,255,65,.1)'); grad.addColorStop(1,'transparent');
    ctx.fillStyle=grad; ctx.fillRect(0,gy,W,18);
    ctx.strokeStyle='rgba(0,255,65,.15)'; ctx.lineWidth=1;
    ctx.setLineDash([5,8]);
    ctx.beginPath(); ctx.moveTo(0,gy); ctx.lineTo(W,gy); ctx.stroke();
    ctx.setLineDash([]);

    // draw threat lines from each live attacker to defender
    attackers.forEach(function(atk){
      if(atk.defeated) return;
      var ax = W*atk.ex, ay = H*atk.ey;
      ctx.save();
      ctx.strokeStyle = atk.color.replace(')',', .08)').replace('rgb','rgba') || 'rgba(255,51,51,.06)';
      ctx.strokeStyle = 'rgba(255,51,51,.06)';
    ...
    attackers.forEach(function(atk){
      if(atk.defeated) return;
      if(frameCount >= atk.lastSpawn + atk.spawnInterval) {
        atk.lastSpawn = frameCount;
        spawnKnifeFrom(atk);
      }
    });
    ...
    attackers.forEach(function(atk){
      ...
      if(atk.defeated) drawDefeatedX(ax, ay+bob);
    });
    ...
    knives.forEach(function(k){
      ...
      if(dist < 38) {
        ...
        // damage the attacker that threw this knife
        ...
        if(atk && !atk.defeated) {
          ...
          if(atk.hp <= 0) {
            ...
            for(var i=0;i<30;i++){
              ...
              particles.push({x:W*atk.ex,y:H*atk.ey,vx:Math.cos(a)*s,vy:Math.sin(a)*s-2,
                life:1,r:2+Math.random()*4,color:['#ff3333','#ff8800','#ffe000'][Math.floor(Math.random()*3)]});
            }
          }
        }
        // impact sparks at defender
        for(var i=0;i<18;i++){
          var a=Math.random()*Math.PI*2, s=2+Math.random()*5;
          particles.push({x:k.tx,y:k.ty,vx:Math.cos(a)*s,vy:Math.sin(a)*s-1,
            life:1,r:2+Math.random()*3,color:['#00ff41','#ffe000','#fff'][Math.floor(Math.random()*3)]});
        }
        ctx.save();
        ctx.strokeStyle='#ffe000'; ctx.lineWidth=2;
        ctx.shadowColor='#ffe000'; ctx.shadowBlur=16;
        ctx.beginPath(); ctx.moveTo(k.tx-18,k.ty-18); ctx.lineTo(k.tx+18,k.ty+18); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(k.tx+18,k.ty-18); ctx.lineTo(k.tx-18,k.ty+18); ctx.stroke();
        ctx.restore();
        // all defeated?
        if(totalDefeated >= attackers.length && !deflectTriggered){
          deflectTriggered=true; battleDone=true;
          setTimeout(triggerDeflect, 200);
        }
      } else {
        k.x += (dx/dist)*k.speed;
        k.y += (dy/dist)*k.speed;
    ...
    ctx.fillText('JAI MAHAKALI · AYOO GORKHALI',44,36);
    ctx.restore();
